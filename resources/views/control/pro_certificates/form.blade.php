    ...
    <form class="pc-form" method="post" action="{{ $editing ? route('certificates.update',['locale'=>app()->getLocale(),'certificate'=>$certificate->id]) : route('certificates.store',['locale'=>app()->getLocale()]) }}">
        ...
        <section class="pc-card"><header class="pc-card-heading"><span class="pc-step" aria-hidden="true">02</span><div><h2>{{ __('certificates.content') }}</h2><p>{{ __('certificates.statement_help') }}</p></div></header>
            <div class="pc-fields">
                ...
                @if(!$editing || (int)$certificate->schema_version>=3)
                <label class="pc-field"><span>{{ __('certificates.credential_basis') }} *</span><select name="credential_basis" required><option value="programme_completion" @selected(old('credential_basis',$certificate->credential_basis??'programme_completion')==='programme_completion')>{{ __('certificates.credential_bases.programme_completion') }}</option><option value="professional_accreditation" @selected(old('credential_basis',$certificate->credential_basis)==='professional_accreditation')>{{ __('certificates.credential_bases.professional_accreditation') }}</option></select><small>{{ __('certificates.credential_basis_help') }}</small></label>
                <label class="pc-field"><span>{{ __('certificates.accreditation_reference') }}</span><input name="accreditation_reference" maxlength="120" dir="ltr" value="{{ old('accreditation_reference',$certificate->accreditation_reference) }}"><small>{{ __('certificates.accreditation_reference_help') }}</small></label>
                <label class="pc-field"><span>{{ __('certificates.accreditation_date') }}</span><input type="date" name="accreditation_date" dir="ltr" value="{{ old('accreditation_date',$certificate->accreditation_date?->format('Y-m-d')) }}"></label>
                @endif
