...
<div class="membership-module">
        ...
        <section class="form-card"><header><div><h2>{{ __('memberships.workflow') }}</h2><p>{{ __('memberships.workflow_notice') }}</p></div></header>
            @if($integrity)
                ...
                @if($credentials->isNotEmpty() && $integrity && in_array($membership->status,['active','suspended'],true) && auth()->user()->canDo('memberships.correct'))
                    <form method="post" enctype="multipart/form-data" action="{{ route('memberships.credentials.revision',['locale'=>app()->getLocale(),'membership'=>$membership->id]) }}">
                        @csrf
                        ...
                        <label>{{ __('memberships.replacement_photo') }}<input type="file" name="photo" accept="image/jpeg,image/png,image/webp"><small>{{ __('memberships.replacement_photo_notice') }}</small></label>
    ...
    <section class="form-card"><header><div><h2>{{ __('memberships.credentials') }}</h2><p>{{ __('memberships.credentials_notice') }}</p></div></header>
        @forelse($credentials as $credential)
            <article class="membership-period">
                ...
                <div><a class="secondary-action" href="{{ route('memberships.credentials.download',['locale'=>app()->getLocale(),'membership'=>$membership->id,'credential'=>$credential->id,'kind'=>'card']) }}">{{ __('memberships.download_card') }}</a> @if(in_array(($credential->payload['template_version'] ?? null), \App\Services\MembershipCredentialPdf::PRINT_IMAGE_TEMPLATES, true))<a class="secondary-action" href="{{ route('memberships.credentials.print-images',['locale'=>app()->getLocale(),'membership'=>$membership->id,'credential'=>$credential->id]) }}">{{ __('memberships.download_card_print_images') }}</a>@endif <a class="secondary-action" href="{{ route('memberships.credentials.download',['locale'=>app()->getLocale(),'membership'=>$membership->id,'credential'=>$credential->id,'kind'=>'certificate']) }}">{{ __('memberships.download_membership_certificate') }}</a> <a class="secondary-action" href="{{ route('memberships.credentials.certificate-print-image',['locale'=>app()->getLocale(),'membership'=>$membership->id,'credential'=>$credential->id]) }}">{{ __('memberships.download_certificate_print_image') }}</a> <a class="secondary-action" target="_blank" rel="noopener noreferrer" href="{{ route('memberships.verify',['token'=>$credential->public_token,'lang'=>app()->getLocale()]) }}">{{ __('memberships.open') }}</a></div>
    ...
    <section class="form-card"><header><div><h2>{{ __('memberships.periods') }}</h2><p>{{ __('memberships.periods_notice') }}</p></div></header>
        @forelse($membership->periods as $period)<article class="membership-period"><div><strong>{{ __('memberships.version') }} {{ $period->version }}</strong><span class="membership-integrity {{ $periodChecks[$period->id]?'is-valid':'is-invalid' }}">{{ __($periodChecks[$period->id]?'memberships.verified':'memberships.integrity_failed') }}</span></div><p><bdi dir="ltr">{{ $period->valid_from->format('Y-m-d') }} → {{ $period->valid_until->format('Y-m-d') }}</bdi></p>@if(isset($period->payload['previous_membership_number']))<p>{{ __('memberships.previous_membership_number') }}: <bdi dir="ltr">{{ $period->payload['previous_membership_number'] }}</bdi></p>@endif<p>{{ __('memberships.approver') }}: {{ $period->approver?->name }}</p><details><summary>{{ __('memberships.fingerprint') }}</summary><code class="membership-hash" dir="ltr">{{ $period->payload_sha256 }}</code><small>{{ __('memberships.hash_notice') }}</small></details>@if(isset($period->payload['correction_of_period_uuid']))<small>{{ __('memberships.corrected_period') }}</small>@endif</article>@empty<p>{{ __('memberships.no_periods') }}</p>@endforelse
    </section>
