...
<head><meta charset="utf-8"><title>{{ in_array($event, ['author_message_received', 'editor_message_received'], true) ? __('journal.editorial_message_email_subject', $payload) : __('journal.mail.subjects.'.$event, $payload) }}</title></head>
...
<div style="max-width:680px;margin:0 auto;padding:32px 18px">
    ...
    <div style="background:#fff;padding:28px 26px;border:1px solid #dbe3e8;border-top:0">
        ...
        <p>{{ in_array($event, ['author_message_received', 'editor_message_received'], true) ? __('journal.editorial_message_email_text', $payload) : __('journal.mail.events.'.$event, $payload) }}</p>
        ...
        @elseif(in_array($event, ['review_assigned', 'review_reminder', 'revision_received', 'new_submission_received', 'review_completed', 'author_message_received', 'editor_message_received'], true))
