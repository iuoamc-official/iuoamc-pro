...
<head>
    ...
    @foreach(['ar', 'en', 'fr'] as $language)
        <link rel="alternate" hreflang="{{ $language }}" href="{{ $page->slug === 'home' ? route('public.home', ['locale' => $language]) : route('public.pages.show', ['locale' => $language, 'publicPage' => $page]) }}">
    @endforeach
        ...
        <div class="public-container header-row">
            ...
            <nav id="public-navigation" class="public-navigation" aria-label="{{ __('public_site.navigation') }}" data-navigation>
                ...
                @foreach($navigation as $item)
                    <a class="{{ $page->is($item) ? 'active' : '' }}" href="{{ route('public.pages.show', ['locale' => app()->getLocale(), 'publicPage' => $item]) }}">{{ $item->localized('navigation_label') }}</a>
                @endforeach
            </nav>
            <div class="header-tools">
                <nav class="public-languages" aria-label="{{ __('public_site.languages') }}">
                    @foreach(['ar' => 'ع', 'en' => 'EN', 'fr' => 'FR'] as $language => $label)
                        <a class="{{ app()->getLocale() === $language ? 'active' : '' }}" hreflang="{{ $language }}" href="{{ $page->slug === 'home' ? route('public.home', ['locale' => $language]) : route('public.pages.show', ['locale' => $language, 'publicPage' => $page]) }}">{{ $label }}</a>
                    @endforeach
                </nav>
                @auth
