...
<div class="membership-module">
    ...
    <section class="form-card">
        <form method="post" action="{{ route('memberships.correction.store',['locale'=>app()->getLocale(),'membership'=>$membership->id]) }}">
            ...
            <label>{{ __('memberships.type') }}<select name="membership_type" required>@foreach($membershipCategories as $value=>$label)<option value="{{ $value }}" @selected(old('membership_type',$membership->membership_type)===$value)>{{ $label }}</option>@endforeach</select></label>
            ...
            <label>{{ __('memberships.replacement_membership_number') }}<input name="replacement_membership_number" maxlength="80" dir="ltr" placeholder="{{ $membership->membership_number }}" value="{{ old('replacement_membership_number') }}"><small>{{ __('memberships.replacement_membership_number_help') }}</small></label>
