...
return new class extends Migration
{
    ...
    {
        if (! Schema::hasTable('journal_submission_contributors')) {
            Schema::create('journal_submission_contributors', function (Blueprint $table): void {
                ...
                $table->unique(['journal_submission_id', 'position'], 'jsc_submission_position_unique');
            });
        } else {
            // MySQL may preserve the table after rejecting the original overlong index name.
            Schema::table('journal_submission_contributors', function (Blueprint $table): void {
                $table->unique(['journal_submission_id', 'position'], 'jsc_submission_position_unique');
            });
        }

        if (! Schema::hasColumn('journal_article_author', 'contribution_roles')) {
        ...
        if (! Schema::hasColumn('journal_reviews', 'conflict_status')) {
        ...
        if (! Schema::hasTable('journal_editorial_messages')) {
            Schema::create('journal_editorial_messages', function (Blueprint $table): void {
                ...
                $table->index(['journal_submission_id', 'sent_at'], 'jem_submission_sent_index');
            });
        }
    }
