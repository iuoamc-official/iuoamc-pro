    ...
    <div class="pc-detail-grid">
        <section class="pc-card"><header class="pc-card-heading"><div><h2>{{ __('certificates.identity') }}</h2><p><bdi>{{ $certificate->organization?->display_name }}</bdi></p></div></header>
            <dl class="pc-facts">
                ...
                @if((int)$certificate->schema_version===2)<div><dt>{{ __('certificate_catalog.type_code') }}</dt><dd><bdi dir="ltr">{{ $certificate->catalog_snapshot['code']??'—' }}</bdi></dd></div><div><dt>{{ __('certificate_catalog.specialization') }}</dt><dd><bdi>{{ ($certificate->specialization!==null && $certificate->specialization!=='') ? $certificate->specialization : '—' }}</bdi></dd></div>@endif
